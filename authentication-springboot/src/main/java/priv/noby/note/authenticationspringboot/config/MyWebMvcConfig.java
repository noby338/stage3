package priv.noby.note.authenticationspringboot.config;
...
import priv.noby.note.authenticationspringboot.interceptor.LoginJwtInterceptor;
        ...
        return new WebMvcConfigurer() {
            ...
            public void addCorsMappings(CorsRegistry registry) {
                registry.addMapping("/**")//允许跨域的接口(controller)
                        .allowedOrigins("http://localhost:8090")//允许可以访问的域(前端)
...
//                        .addPathPatterns("/student/afterSessionLogin")
//                        .excludePathPatterns("/user/sessionLogin");
                //endregion "session配置"
                // region "jwt配置"
                registry.addInterceptor(new LoginJwtInterceptor())
                        .addPathPatterns("/student/afterJwtLogin")
