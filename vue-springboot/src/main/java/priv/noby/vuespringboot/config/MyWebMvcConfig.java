...
import priv.noby.vuespringboot.interceptor.LoginSessionInterceptor;
...
public class MyWebMvcConfig {
    ...
    public WebMvcConfigurer webMvcConfigurer() {
        return new WebMvcConfigurer() {
             ...
             * 跨域映射，跨域的全局配置
             *
             * 添加该配置后，后端响应给服务器的响应头中会携带，Access-Control-Allow-Origin头，如此浏览器不再绝收该响应
             */
            @Override
            public void addCorsMappings(CorsRegistry registry) {
                registry.addMapping("/**")//允许跨域的接口(controller)
                        .allowedOrigins("*")//允许可以访问的域(前端)
                        .allowCredentials(true)//允许来自其他域携带凭证，Cookie存在此凭证中
